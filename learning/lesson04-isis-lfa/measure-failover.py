...
def lfa_is_preinstalled() -> bool:
    entry = route_entry()
    if entry is None or route_nexthop() != PRIMARY:
        return False
    backups = {
    ...
    linked = any(hop.get("backupIndex") == [0] for hop in entry.get("nexthops", []))
    return backups == {(BACKUP, "eth3")} and linked
...
def wait_for_recovery(timeout: float) -> dict[str, float]:
    ...
    checks = {
        ...
        "lfa_rearmed_ms": lfa_is_preinstalled,
    }
    with ThreadPoolExecutor(max_workers=len(checks)) as pool:
        futures = {
            name: pool.submit(observe_transition, predicate, started, timeout)
...
def main() -> int:
    ...
    report: dict[str, object] = {
        ...
        "protection": {
            "type": "classic_lfa",
            "protected_interface": "r1:eth2",
            "protected_prefix": PREFIX,
            "primary": "10.0.12.1 dev eth2",
            "backup": "10.0.13.1 dev eth3",
            "backup_preinstalled_before_fault": False,
        },
    ...
    try:
        ...
        if not lfa_is_preinstalled():
            raise ExperimentError("the expected LFA backup is not preinstalled; run 'make test' first")
        report["protection"]["backup_preinstalled_before_fault"] = True
        ...
        if route_nexthop() != BACKUP:
            raise ExperimentError("traffic did not remain on the LFA path during the fault")
        ...
        print("PASS: classic LFA backup was linked to the primary before the fault")
        ...
        print(f"MEASURE: backup FIB observed = {transitions['fib_switch_ms']:.1f} ms")
    ...
    try:
        ...
        print(f"PASS: LFA protection re-armed after {recovery['lfa_rearmed_ms']:.1f} ms")
